"""A program to find schedule conflicts
Amelia Bates, CIS 211, 2022-04-04
"""
...
class Appt:
    ...
    def __init__(self, start: datetime, finish: datetime, desc: str):
        ...
        self.start = start  # When the appointment starts
        self.finish = finish  # When the appointment ends
        self.desc = desc  # desc = description
    ...
    def __lt__(self, other: 'Appt') -> bool:
        """Less than means that it starts before"""
        return self.start < other.start or self.finish < other.finish

    def __gt__(self, other: 'Appt') -> bool:
        """Greater than means that it starts after"""
        return self.start > other.start or self.finish > other.finish

    def overlaps(self, other: 'Appt') -> bool:
        """Is there a non-zero overlap between these periods?"""
        if self.start.__eq__(other.finish):
            return False  # There is not an overlap
        if other.start.__eq__(self.finish):
            return False
        if self.start.__gt__(other.finish):
            return False
        if self.finish.__lt__(other.start):
            return False
    ...
    def intersect(self, other: 'Appt') -> 'Appt':
        ...
        '''2018-03-15 15:00 15:30 | Early afternoon nap and Coffee break'''  # The format wanted
...
class Agenda:
    ...
    def __len__(self):
        """ Returns the length of the list element"""
    ...
    def start_time(appt: Appt) -> datetime:
        """Extracts the start time"""
        return appt.start

    def append(self, appointment: "Appt"):
        """Appends item to list"""
        return self.elements.append(appointment)
    ...
    def conflicts(self) -> 'Agenda':
        ...
        """
        self.sort()  # Sorts by start time
        a = Appt
        conflict = Agenda()  # New list of just conflicts
        for i in range(len(self.elements)):
            for j in range(i + 1, len(self.elements)):
                if a.overlaps(self.elements[i], self.elements[j]):
                    y = a.intersect(self.elements[i], self.elements[j])
                    conflict.append(y)  # If there is a conflict, it is added to the list of conflicts
                else:
                    break
        return conflict
...
if __name__ == "__main__":
    ...
    appt1 = Appt(datetime(2018, 3, 15, 13, 30), datetime(2018, 3, 15, 15, 30), "Early afternoon nap")
    appt2 = Appt(datetime(2018, 3, 15, 15, 00), datetime(2018, 3, 15, 16, 00), "Coffee break")
    if appt1.overlaps(appt2):  # moved this before appt2 > appt1 as it didn't print correctly
        print("Oh no, a conflict in the schedule!")
        print(appt1.intersect(appt2))
    elif appt2 > appt1:
        print(f"appt1 '{appt1}' was over when appt2 '{appt2}'  started")
