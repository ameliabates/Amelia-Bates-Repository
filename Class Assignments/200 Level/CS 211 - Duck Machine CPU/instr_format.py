...
class Instruction(object):
    ...
    def encode(self) -> int:
        ...
        op_int = self.op.value
        cond_int = self.cond.value
        ...
        word = cond_field.insert(cond_int, word)
        word = op_field.insert(op_int, word)
...
def decode(word: int) -> Instruction:
    ...
    return Instruction(OpCode(op_field.extract(word)), CondFlag(cond_field.extract(word)),
                       reg_target_field.extract(word),
                       reg_src1_field.extract(word), reg_src2_field.extract(word),
