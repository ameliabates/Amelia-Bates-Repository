...

{
  /* TODO */
    ...
    *csr_vals = (double*) malloc(sizeof(double) * nnz);
    unsigned int sum = 0;
    unsigned int* row_num = (unsigned int*) calloc(m, sizeof(unsigned int));
...
for (int i = 0; i < m; i++)
{
 res[i] = 0.0;
        for (int j = 0; j < n; j++)
        {
                res[i] += csr_vals[csr_row_ptr[i]]* vector_x[csr_col_ind[j]];
                printf("Here is %0.10f\n", res[i]); //delete this
	...
	for (int i = 0; i < m; i++)
	{
		...
		double sum = 0.0;
			...
			//printf("%f\n"
                	...
                	//printf("Here is %0.10f\n", res[i]); //delete this
                	//printf("%f and %f at %d\n", csr_vals[j], vector_x[csr_col_ind[j]], i);
        	}
		//printf("Here is %0.10f\n", res[i]); //delete this
		//res[i] = sum;
		//printf("Here is %0.10f\n", res[i]); //delete this	
